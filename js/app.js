...
const setUpThreads = (data) => {
  ...
  threadsList.innerHTML = (count == 0)
    ? noContent('No codes Yet', 'Be the first to post one!')
    : html;
  ...
  db.collection('comments')
    ...
    .then(snapshot => {
      ...
      commentsList.innerHTML = (count == 0)
        ? noContent('No Comments Yet', 'Be the first to share what you think about the code!')
        : html;
...
$$('.new-thread-dialog').on('click', function () {
  app.dialog.create({
    content: '<div class="page-content login-screen-content"> <div class="block-title">New Code</div> <form class="list" id="create-thread-form"> <div class="list" id="dialog-list"> <ul> <li class="item-content item-input"> <div class="item-inner"> <div class="item-input-wrap"> <input type="text" id="title" name="title" placeholder="Code Title" required validate/> </div> </div> </li> <li class="item-content item-input"> <div class="item-inner"> <div class="item-input-wrap"> <textarea id="description" name="description" placeholder="Description + Code" required validate></textarea> </div> </div> </li> <li class="item-content item-input"> <div class="item-inner"> <input type="file" id="thread-img-upload" required validate> </li> </ul> </div> <div class="row display-flex justify-content-center"> <a class="transbtn" id="create-thread" href="#">Create Code</a> <a class="transbtn" id="cancel-thread" href="#">Cancel</a> </div> </form> </div>',
    cssClass: 'dialog'
  }).open();
...
$$(document).on('click', '.delete-thread-dialog', function () {
  ...
  app.dialog.confirm(' Are you sure you want to delete the code?', '', function () {
